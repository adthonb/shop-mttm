...
if ( in_array( 'woocommerce/woocommerce.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
    ...
    function woocommerce_loaded() {
        // Hook in
        add_filter( 'woocommerce_checkout_fields' , 'mttm_custom_checkout_fields', 9999 );
    ...
    function mttm_parcel_sticker_bulk_actions( $actions ) {
        $actions['parcel_sticker'] = __('Download Parcel Sticker', 'misson-tt-moon');
        return $actions;
    }
    ...
    function mttm_handle_parcel_sticker_bulk_actions( $redirect_to, $action, $post_ids ) {
        ...
        $mpdf = new \Mpdf\Mpdf([
            /*'fontDir' => array_merge($fontDirs, [__DIR__ . '/assets/font']),
            'fontdata' => $fontData + [
                'sarabun' => [
                    'R' => 'THSarabunNew.ttf',
        ...
        foreach($post_ids as $key => $post_id) {
            ...
            $sub_city = get_post_meta($post_id, '_shipping_sub_city', true);
            ...
            if ($states === 'TH-10') {
                $address = $company . ' ' . $order->get_shipping_address_1() . ' แขวง' . $sub_city . ' เขต' . $order->get_shipping_city() . ' ' . $states_list[$states];
            } else {
                $address = $company . ' ' . $order->get_shipping_address_1() . ' ตำบล' . $sub_city . ' อำเภอ' . $order->get_shipping_city() . ' ' . $states_list[$states];
            }
